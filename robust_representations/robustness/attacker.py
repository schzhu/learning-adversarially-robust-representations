...
class AttackerModel(ch.nn.Module):
    ...
    def __init__(self, model, dataset):
        ...
        # self.attacker.normalizer = helpers.InputNormalize(dataset.mean, dataset.std)
        # self.model = model
        self.attacker = Attacker(model, dataset)

    def forward(self, inp, target=None, make_adv=False, with_latent=False,
                fake_relu=False, no_relu=False, with_image=True,
                get_feature_only=False, **attacker_kwargs):
        """
        ...
        if with_image:
            normalized_inp = self.attacker.normalizer(inp) # todo typo normalize
            ...
            if get_feature_only:
                output = self.attacker.model.forward_get_feature_layer_3(normalized_inp, fake_relu)
                # output = self.attacker.model.forward_get_feature(normalized_inp)
            else:
                output = self.attacker.model(normalized_inp, with_latent=with_latent,
    ...
    # def forward_disc(self, inp, target=None, make_adv=False, with_latent=False,
    #                  fake_relu=False, no_relu=False, with_image=True,
    #                  renew_perm=True, **attacker_kwargs):
    #     if renew_perm:
    #         self.attacker.model.renew_perm(inp.shape[0])
    #
    #     self.attacker.model.testee.eval()
    #     adv = inp
    #     if make_adv:
    #         assert target is not None
    #         prev_training = bool(self.training)
    #         self.attacker.model.disc.eval()
    #         # put fixed x into target
    #         # inp_as_target = inp.clone()
    #         adv = self.attacker(inp, target, **attacker_kwargs, should_normalize=False)
    #         if prev_training:
    #             self.attacker.model.disc.train()
    #
    #         # inp = adv
    #
    #     if with_image:
    #         # normalized_inp = self.attacker.normalizer(inp)
    #         # normalized_adv = self.attacker.normalizer(adv)
    #
    #         if no_relu and (not with_latent):
    #             print("WARNING: 'no_relu' has no visible effect if 'with_latent is False.")
    #         if no_relu and fake_relu:
    #             raise ValueError("Options 'no_relu' and 'fake_relu' are exclusive")
    #
    #         output = self.attacker.model.forward_fix_x(inp=adv,
    #                                           inp_fixed=inp,
    #                                           fake_relu=False)
    #     else:
    #         output = None
    ...
    #         self.attacker.model.testee.train()
    #
    #     return (output, inp)


    def forward_custom(self, input, target=None, make_adv=False,
                       with_latent=False, fake_relu=False,
                       no_relu=False, with_image=True,
                       loss_type=None,
                       detach=False,
                       enc_in_eval=True,
                       **attacker_kwargs):
        '''
        forward propagation of godim (dim) or goh (classification),
            with normal/adversarial loss

        Parameters:
            input (ch.tensor) : input to do inference on [N x input_shape] (e.g. NCHW).
            target (ch.tensor) : pointed to input for encoding, otherwise labels.
            loss_type : 'dim' (mi estimation) or 'cla' (classification)
            detach : compute classification loss for top classifier
                without back-propagate to enc.
            enc_in_eval : DO NOT UPDATE THE BATCH-NORMALIZATION TERMS IN FORWARD
        '''
        if make_adv:
            prev_training = bool(self.training)
            self.eval()
            # adv_target = target
            adv = self.attacker(input, target, **attacker_kwargs, should_normalize=True)
            if prev_training:
                self.train()
        ...
        if with_image:
            normalized_input = self.attacker.normalize(input)
            # normalized_adv = self.attacker.normalizer(adv)
            ...
            output = self.attacker.model.forward(input=normalized_input,
                                                 target=target,
                                                 loss_type=loss_type,
                                                 detach=detach,
    ...
    # def forward_2_enc(self, inp, make_adv=False, with_latent=False,
    #                  fake_relu=False, no_relu=False, with_image=True,
    #                  no_update_enc=True, **attacker_kwargs):
    #
    #     # self.attacker.model.refresh_prior_sample((self.attacker.model.latent_size,))
    #
    #     if make_adv:
    #         prev_training = bool(self.training)
    #         self.eval()
    #         adv_target = inp
    #         adv = self.attacker(inp, adv_target, **attacker_kwargs, should_normalize=True)
    #         if prev_training:
    #             self.train()
    #
    #         inp = adv
    #
    #     if with_image:
    #         normalized_inp = self.attacker.normalize(inp)
    #         # normalized_adv = self.attacker.normalizer(adv)
    #
    #         if no_relu and (not with_latent):
    #             print("WARNING: 'no_relu' has no visible effect if 'with_latent is False.")
    ...
    #             raise ValueError("Options 'no_relu' and 'fake_relu' are exclusive")
    #
    #         output = self.attacker.model.forward_enc(input=normalized_inp, no_update_enc=no_update_enc)
    #         # output = self.attacker.model.kernel_enc(input=inp, no_update_enc=no_update_enc)
    #     else:
    #         output = None
    #
    #     return output
    ...
    # def forward_2_sup(self, inp, target=None, make_adv=False, with_latent=False,
    #                  fake_relu=False, no_relu=False, with_image=True, detach=False,
    #                  no_update_enc=True, **attacker_kwargs):
    #
    #     if make_adv:
    #         assert target is not None
    ...
    #             raise ValueError("Options 'no_relu' and 'fake_relu' are exclusive")
    #
    #         output = self.attacker.model.forward_sup(input=normalized_inp, target=target,
    #                                         detach=detach, no_update_enc=no_update_enc)
    #     else:
    #         output = None
    #
    #
