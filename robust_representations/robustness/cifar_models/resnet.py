...
class ResNet(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes=10, feat_scale=1, wm=1):
        ...
        # self.random_layer = nn.Linear(feat_scale*widths[3]*block.expansion, num_classes)
    ...
    def forward(self, x, with_latent=False, fake_relu=False, no_relu=False):
        ...
        # random_out =  self.random_layer(pre_out)
        if with_latent:
            return final, pre_out
        return final

    def forward_get_feature_layer_1(self, x):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        return out

    def forward_get_feature_layer_3(self, x, fake_relu=False):
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out, fake_relu=fake_relu)
        return out
